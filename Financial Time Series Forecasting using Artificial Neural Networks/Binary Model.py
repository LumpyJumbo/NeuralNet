...
for i in range(window, split):
    training_reality.append(data[i, 4]-data[i-1, 4])

training_reality = np.array(training_reality, dtype=float)
labels = np.where(training_reality>0,1,0)
...
for i in range(split+window, split2):
    validation_changes.append(data[i, 4]-data[i-1, 4])
    naive_predictions.append(0.5)

validation_changes = np.array(validation_changes, dtype=float)
reality = np.where(validation_changes>0,1,0)
...
for i in range(split2+window, data.shape[0]):
    test_changes.append(data[i, 4]-data[i-1, 4])

test_changes = np.array(test_changes, dtype=float)
test_labels = np.where(test_changes>0,1,0)
...
plt.pyplot.figure(1)
plt.pyplot.plot(reality, color='b', linestyle='None', marker='o', markersize=5.0)
plt.pyplot.plot(predictions, color='r', linestyle='None', marker='o', markersize=5.0)
plt.pyplot.show()
...
for i in range(0,predictions.shape[0]):
    ...
    assets = [asset_value*(1-predictions[i]),asset_value*predictions[i]/data[split+window-1+i,4]]
